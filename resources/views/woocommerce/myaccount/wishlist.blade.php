{{--
  ...
  Особистий кабінет — контент вкладки Wishlist.

  @see woocommerce_account_wishlist_endpoint
--}}
@php
  if (! defined('ABSPATH')) {
      exit;
...
@if (empty($wishlist_ids))
  ...
  <div class="text-center py-8">
    ...
    <p class="text-sm text-gray-600 mb-6">
      {{ __('У вас ще немає вподобаних товарів. Додайте їх, натиснувши на серце в каталозі.', 'solidshop') }}
    </p>
    <a
...
@else
  ...
  @if ($wishlist_query->have_posts())
    <div class="grid grid-cols-2 md:grid-cols-4 gap-6" id="wishlist-grid">
  ...
  @else
    <div class="text-center py-8">
      <p class="text-sm text-gray-600 mb-6">
        {{ __('Товари з вашого списку більше недоступні.', 'solidshop') }}
      </p>
      <a
        href="{{ esc_url($shop_url) }}"
        class="inline-flex items-center justify-center bg-black hover:bg-gray-800 text-white font-bold py-3 px-6 rounded-none transition-colors no-underline"
